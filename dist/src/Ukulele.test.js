...
const UKULELE_TUNING = ['D', 'G', 'B', 'E'];
function getFretNotes(barreFret, upToFret) {
    let fretNotes = [];
...
function getScaleDegrees(notes, key) {
    ...
    let scaleDegrees = notes.map(note => {
        let degree = scaleNotes.indexOf(note);
        return degree === -1 ? 'X' : degree + 1;
    });
    return scaleDegrees;
}
function findChords(availables) {
    ...
    for (let fretCombination of allFretCombinations) {
        ...
        let chord = getChord(degrees);
        if (chord[1] == 'invalid')
            continue;
        let r = [chord[0], degrees, fretCombination];
        chords.push(r);
    }
    return chords;
}
function generateFretCombinations(scaleDegrees, fretCombination = []) {
...
function scaleDegreesToSemitones(scaleDegrees) {
    const majorScaleSemitones = [0, 2, 4, 5, 7, 9, 11];
    return scaleDegrees.map(degree => degree === 'X' ? 'X' : majorScaleSemitones[(degree - 1) % 7]);
}
function semitoneToNoteInCMajor(semitone) {
    const majorScaleSemitones = [0, 2, 4, 5, 7, 9, 11];
    const majorScaleNotes = ['C', 'D', 'E', 'F', 'G', 'A', 'B'];
    let index = majorScaleSemitones.indexOf(semitone);
    return majorScaleNotes[index];
}
function getChord(degrees) {
    let semitones = scaleDegreesToSemitones(degrees);
    let numericSemitones = semitones.filter(semitone => semitone !== 'X');
    ...
    for (let j = 0; j < sortedSemitones.length; j++) {
        ...
        let intervals = sortedSemitones.map(semitone => (semitone - rootSemitone + 12) % 12)
            .sort((a, b) => a - b);
        let chordType = Object.keys(CHORDS).find(chordType => arraysEqual(CHORDS[chordType], intervals));
...
function getOpenChordPosition(frets) {
...
const chord = tonal_1.Chord.detect(["D", "F#", "A", "C"]);
console.log(chord); // Outputs: [ 'D7' ]
//# sourceMappingURL=Ukulele.test.js.map